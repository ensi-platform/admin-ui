...
- поиск — кнопка рядом со сворачиванием: затемнение и небольшое окно выше центра, под полем разделы со ссылкой и путь к ним; пустой запрос оставляет под полем пустую область; Escape или нажатие на затемнение закрывает окно; свёрнутая полоска при этом не разворачивается
